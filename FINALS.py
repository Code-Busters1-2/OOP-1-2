...
connector.execute(
"CREATE TABLE IF NOT EXISTS SCHOOL_MANAGEMENT (STUDENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT, DOB TEXT, COURSE TEXT)"
)

# Creating the functions
def reset_fields():
   global name_strvar, email_strvar, contact_strvar, gender_strvar, dob, course_strvar

   for i in ['name_strvar', 'email_strvar', 'contact_strvar', 'gender_strvar', 'course_strvar']:
       exec(f"{i}.set('')")
...
def add_record():
   ...
   DOB = dob.get_date()
   course = course_strvar.get()

   if not name or not email or not contact or not gender or not DOB or not course:
       mb.showerror('Error!', "Please fill all the missing fields!!")
   else:
       try:
           connector.execute(
           'INSERT INTO SCHOOL_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, DOB, STREAM) VALUES (?,?,?,?,?,?)', (name, email, contact, gender, DOB, course)
           )
           connector.commit()
...
def view_record():
   ...
   dob.set_date(date); course_strvar.set(selection[6])
...
cf_bg = 'Cyan' # bg color for the center_frame
...
Label(left_frame, text="Course", font=labelfont, bg=lf_bg).place(relx=0.3, rely=0.7)
...
Entry(left_frame, width=19, textvariable=course_strvar, font=entryfont).place(x=20, rely=0.75)
...
Label(right_frame, text='Students Records', font=headlabelfont, bg='Navy Blue', fg='LightCyan').pack(side=TOP, fill=X)

tree = ttk.Treeview(right_frame, height=100, selectmode=BROWSE,
                   columns=('Student ID', "Name", "Email Address", "Contact Number", "Gender", "Date of Birth", "Course"))
